...
export async function POST(request:Request){
 ...
 const loginKey=phone||identifier.toLowerCase();\n const now=new Date(),attempt=await DB.prepare("SELECT failed_count AS failedCount,locked_until AS lockedUntil FROM member_login_attempts WHERE email=?").bind(loginKey).first<{failedCount:number;lockedUntil:string}>();
