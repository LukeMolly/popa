...
export async function GET() {
  ...
  try {
    ...
    const [members,payments,audits,recoveryRequests,settings] = await Promise.all([
      ...
      db.prepare("SELECT id,member_id AS memberId,amount,reference,status,note,created_at AS createdAt,reviewed_at AS reviewedAt,reviewed_by AS reviewedBy FROM payments ORDER BY created_at DESC").all(),
