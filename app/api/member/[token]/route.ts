...
export async function GET(_request:Request,{params}:Context){
 try{
  ...
  const [payment,settings,notifications]=await Promise.all([
   ...
   env.DB.prepare("SELECT id,title,body,reply,replied_at AS repliedAt,created_at AS createdAt FROM member_notifications WHERE member_id=? ORDER BY created_at DESC LIMIT 50").bind(member.id).all()
  ]);
  return Response.json({member:{...member,reminderDays:Number(settings?.reminderDays)||7},payment,notifications:notifications.results},{headers:{"Cache-Control":"no-store"}});
 }catch(e){console.error(e);return Response.json({error:"Membership unavailable."},{status:500})}
}
export async function POST(request:Request,{params}:Context){
 try{
  const {token}=await params;
  const authorised=await authorisedMember(token);
  if(!authorised)return Response.json({error:"Member sign-in required."},{status:403});
  if((request.headers.get("content-type")||"").includes("application/json")){
   const data=await request.json() as Record<string,unknown>;
   if(data.action!=="notification-reply")return Response.json({error:"Unknown action."},{status:400});
   const id=Number(data.id),reply=typeof data.reply==="string"?data.reply.trim().slice(0,1000):"";
   if(!Number.isInteger(id)||!reply)return Response.json({error:"Enter your response."},{status:400});
   const result=await env.DB.prepare("UPDATE member_notifications SET reply=?,replied_at=? WHERE id=? AND member_id=?").bind(reply,new Date().toISOString(),id,authorised.id).run();
   return result.meta.changes?Response.json({ok:true}):Response.json({error:"Notification not found."},{status:404});
  }
