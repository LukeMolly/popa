...
export default function MemberPortal({params}:{params:Promise<{token:string}>}){
 ...
 async function submit(e:React.FormEvent<HTMLFormElement>){
  ...
  try{
   ...
   setPayment({id:result.id,amount:Number(result.amount||settings.membershipFee),reference:String(body.get("reference")||""),status:"submitted",note:"",createdAt:new Date().toISOString()});
   form.reset();
   ...
   {loading?<section className="member-panel">Loading your membership…</section>:!member?<section className="member-panel" role="alert">{error||"Member record not found."}</section>:<>
    ...
    <div className="member-columns">
     ...
     <section className="member-panel">
      ...
      <h2>Payment</h2>{payment?<div className="payment-state"><span className={"badge "+payment.status}>{payment.status}</span><p>P{payment.amount} · Submitted {when(payment.createdAt)}</p>{payment.reference&&<p>Reference: {payment.reference}</p>}{payment.note&&<p>Membership office note: {payment.note}</p>}</div>:<p className="muted">No payment submission is recorded yet.</p>}
      {effectiveStatus!=="active"&&(effectiveStatus==="expired"||!payment||payment.status==="rejected")&&<form onSubmit={submit} className="proof-form"><label>Payment reference (optional)<input name="reference" maxLength={100} placeholder="Transaction reference"/></label><label>Proof of payment<input name="proof" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" required/></label><p className="muted">P{settings.membershipFee} · JPG, PNG, WebP or PDF · maximum 5 MB</p><button disabled={busy} className="primary">{busy?"Uploading…":"Submit proof for review"}</button></form>}
