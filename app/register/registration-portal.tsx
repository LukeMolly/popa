 ...
 return <main className="portal-shell">
  ...
  <Tabs value={active} onValueChange={setActive} className="workspace">
    ...
    <section className="panel form-panel">
     ...
     <form onSubmit={submit}>
      <div className="form-grid">
       <Field label="Full name"><Input name="fullName" required placeholder="First name and surname" autoComplete="name"/></Field>
       <Field label="Omang / Passport number"><Input name="idNumber" required placeholder="Identity number"/></Field>
       <Field label="Date of birth" hint="Select your date from the calendar"><div className="date-picker-field"><Input ref={dobRef} name="dateOfBirth" required type="date" min="1900-01-01" max={latestBirthDate} autoComplete="bday"/><button type="button" onClick={openDatePicker} aria-label="Open date of birth calendar"><CalendarDays/></button></div></Field>
       <Field label="Place of birth"><Input name="placeOfBirth" required placeholder="Town or village"/></Field>
       <Field label="Email address"><Input name="email" required type="email" defaultValue={accountEmail} placeholder="name@example.com"/></Field>
       <Field label="Mobile number"><Input name="phone" required type="tel" placeholder="+267 7X XXX XXX"/></Field>
       <Field label="Create password"><Input name="password" required type="password" minLength={8} autoComplete="new-password" placeholder="8+ characters, letter and number"/></Field>
       <Field label="Confirm password"><Input name="confirmPassword" required type="password" minLength={8} autoComplete="new-password" placeholder="Repeat password"/></Field>
      ...
      <Button type="submit" size="lg" className="submit-button" disabled={busy}>{busy?"Submitting…":"Submit registration"}<ChevronRight/></Button>
     </form>
    </section>
   ...
   <TabsContent value="membership" className="member-view">
    ...
    <section className="panel status-panel"><span className="eyebrow">MEMBERSHIP STATUS</span><h2>{result?"We are verifying your payment":"Start your membership"}</h2><p>{result?"Your application has been saved. The membership office will confirm your payment and activate your membership.":"Complete the registration form and upload proof of your once-off P200 season payment."}</p><div className="status-row"><FileCheck2/><span><small>Payment proof</small><strong>{result?"Received":"Not submitted"}</strong></span></div><div className="status-row"><CalendarDays/><span><small>Season expiry</small><strong>{result?new Date(result.expiresAt).toLocaleDateString("en-BW",{day:"2-digit",month:"long",year:"numeric"}):expiry}</strong></span></div>{!result&&<Button onClick={()=>setActive("register")}>Register now</Button>}</section>
   </TabsContent>
  </Tabs>
