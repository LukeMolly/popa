...
export default function MemberRegistry({adminRole}:{adminRole:string}){
   ...
   {selected?<>
    ...
    <div className="detail-grid">
     <section className="panel">
      ...
      <hr style={{margin:"20px 0",border:0,borderTop:"1px solid #e3e7ef"}}/>
      <h2>Edit KYC</h2>
      <form className="proof-form" onSubmit={async e=>{e.preventDefault();const f=new FormData(e.currentTarget);const ok=await send({action:"kyc-update",id:selected.id,firstName:f.get("firstName"),lastName:f.get("lastName"),email:f.get("email"),phone:f.get("phone"),idNumber:f.get("idNumber"),dateOfBirth:f.get("dateOfBirth"),placeOfBirth:f.get("placeOfBirth"),membershipLocation:f.get("membershipLocation"),gender:f.get("gender")},"KYC updated");if(ok){const updated={...selected,firstName:String(f.get("firstName")||""),lastName:String(f.get("lastName")||""),email:String(f.get("email")||""),phone:String(f.get("phone")||""),idNumber:String(f.get("idNumber")||""),dateOfBirth:String(f.get("dateOfBirth")||""),placeOfBirth:String(f.get("placeOfBirth")||""),membershipLocation:String(f.get("membershipLocation")||""),gender:String(f.get("gender")||"")};setSelected(updated)}}}>
       <div className="form-grid"><label>First name<input name="firstName" required defaultValue={selected.firstName}/></label><label>Last name<input name="lastName" required defaultValue={selected.lastName}/></label><label>Email<input name="email" type="email" defaultValue={selected.email}/></label><label>Phone<input name="phone" type="tel" defaultValue={selected.phone}/></label><label>ID / Passport<input name="idNumber" defaultValue={selected.idNumber||""}/></label><label>Date of birth<input name="dateOfBirth" type="date" defaultValue={selected.dateOfBirth||""}/></label><label>Place of birth<input name="placeOfBirth" defaultValue={selected.placeOfBirth||""}/></label><label>Region<input name="membershipLocation" defaultValue={selected.membershipLocation||""}/></label><label>Gender<input name="gender" defaultValue={selected.gender||""}/></label></div>
       <button disabled={busy} className="primary">Save KYC changes</button>
      </form>
      <div className="form-actions" style={{marginTop:16,flexWrap:"wrap"}}>
       {selected.status!=="archived"?<button disabled={busy} className="secondary" onClick={async()=>{if(!window.confirm("Archive "+selected.firstName+" "+selected.lastName+"? The record and history will be retained."))return;const ok=await send({action:"archive",id:selected.id},"Member archived");if(ok)setSelected({...selected,status:"archived"})}}>Archive member</button>:<button disabled={busy} className="secondary" onClick={async()=>{const ok=await send({action:"restore",id:selected.id},"Member restored to pending");if(ok)setSelected({...selected,status:"pending"})}}>Restore member</button>}
       {adminRole==="executive"&&<button disabled={busy} className="secondary" onClick={async()=>{const reason=window.prompt("Permanent deletion is irreversible. Enter the reason for deleting this member:");if(!reason?.trim())return;if(!window.confirm("Permanently delete "+selected.firstName+" "+selected.lastName+"? This cannot be undone."))return;const ok=await send({action:"delete",id:selected.id,reason},"Member permanently deleted");if(ok)setSelected(null)}}>Permanently delete</button>}
      </div>
     </section>
