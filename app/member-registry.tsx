...
import {ArrowLeft,Copy,Download,Plus,Search,Users} from "lucide-react";
...
type Member={
 ...
 idNumber?:string;dateOfBirth?:string;placeOfBirth?:string;membershipLocation?:string;gender?:string;
};
type Payment={id:string;memberId:string;amount:number;reference:string;status:string;note:string;createdAt:string;reviewedAt:string};
type Data={members:Member[];payments:Payment[]};
const empty:Data={members:[],payments:[]};
...
export default function MemberRegistry(){
 ...
 const load=useCallback(async()=>{try{const r=await fetch("/api/data",{cache:"no-store"});const v=await r.json() as Data&{error?:string};if(!r.ok)throw Error(v.error||"Could not load records");setData(v);setError("")}catch(e){setError(e instanceof Error?e.message:"Could not load records")}finally{setLoading(false)}},[]);
 ...
 const latestPaymentByMember=useMemo(()=>{
  ...
  for(const p of data.payments)if(!map.has(p.memberId))map.set(p.memberId,p);
  return map;
 },[data.payments]);
 ...
 const list=useMemo(()=>data.members.filter(m=>{
  const hay=(m.id+" "+m.firstName+" "+m.lastName+" "+m.phone+" "+m.email+" "+(m.membershipLocation||"")).toLowerCase();
 ...
 const totals=useMemo(()=>{
  const count=(s:string)=>data.members.filter(m=>effectiveStatus(m)===s).length;
 ...
 function downloadCsv(){
  ...
  const url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download="township-rollers-membership-report.csv";a.click();URL.revokeObjectURL(url);
 }
  ...
  <main className="main">
   ...
   {selected?<>
    ...
    <div className="detail-grid">
     <section className="panel">
      ...
      <dl className="details">
       <div><dt>Member ID</dt><dd>{selected.id}</dd></div>
       <div><dt>Email</dt><dd>{selected.email||"—"}</dd></div>
       <div><dt>Phone</dt><dd>{selected.phone||"—"}</dd></div>
       <div><dt>ID / Passport</dt><dd>{selected.idNumber||"—"}</dd></div>
       <div><dt>Date of birth</dt><dd>{selected.dateOfBirth||"—"}{selected.dateOfBirth?" · "+ageFromDob(selected.dateOfBirth)+" yrs":""}</dd></div>
       <div><dt>Region</dt><dd>{selected.membershipLocation||"—"}</dd></div>
       <div><dt>Gender</dt><dd>{selected.gender||"—"}</dd></div>
       <div><dt>Expiry</dt><dd>{selected.expiresAt||"Not set"}</dd></div>
      </dl>
      <label className="field">Membership status
       <select value={selected.status} onChange={async e=>{const status=e.target.value;if(await send({action:"status",id:selected.id,status},"Status updated"))setSelected({...selected,status})}}>
      ...
      <form className="expiry-edit" onSubmit={async e=>{e.preventDefault();const expiresAt=String(new FormData(e.currentTarget).get("expiresAt")||"");if(await send({action:"expiry",id:selected.id,expiresAt},"Expiry date updated"))setSelected({...selected,expiresAt})}}>
       <label className="field">Expiry date<input name="expiresAt" type="date" key={selected.expiresAt} defaultValue={selected.expiresAt||""} required/></label>
       <button className="primary">Save expiry date</button>
      </form>
     </section>
    ...
    <section className="panel">
     ...
     <div className="table-wrap"><table><thead><tr><th>Date</th><th>Amount</th><th>Reference</th><th>Status</th><th>Note</th></tr></thead><tbody>
      {data.payments.filter(p=>p.memberId===selected.id).map(p=><tr key={p.id}><td>{new Date(p.createdAt).toLocaleString("en-BW")}</td><td>P{p.amount}</td><td>{p.reference||"—"}</td><td><Badge status={p.status}/></td><td>{p.note||"—"}</td></tr>)}
     </tbody></table>{!data.payments.some(p=>p.memberId===selected.id)&&<div className="empty">No payments recorded.</div>}</div>
    </section>
   </>:<>
    <div className="page-head"><div><p className="eyebrow">MEMBERSHIP DASHBOARD</p><h1>Members</h1><p className="sub">Registration, payment status, member analytics and reporting.</p></div><div style={{display:"flex",gap:10,flexWrap:"wrap"}}><button className="secondary" onClick={downloadCsv}><Download size={18}/> Download report</button><button className="primary" onClick={()=>setShowForm(!showForm)}><Plus size={18}/> Add member</button></div></div>

    <div className="stats">
     <div><small>TOTAL MEMBERS</small><strong>{totals.total}</strong></div>
     <div><small>ACTIVE</small><strong>{totals.active}</strong></div>
     <div><small>PENDING</small><strong>{totals.pending}</strong></div>
    </div>
    <div className="stats">
     <div><small>SUSPENDED</small><strong>{totals.suspended}</strong></div>
     <div><small>EXPIRED</small><strong>{totals.expired}</strong></div>
     <div><small>APPROVED REVENUE</small><strong>P{totals.revenue.toLocaleString("en-BW")}</strong><span className="muted">{totals.pendingPayments} payment(s) awaiting review</span></div>
    </div>
    ...
    <section className="panel"><div className="panel-head"><div><h2>Member directory</h2><p>{list.length} records</p></div></div><div className="table-wrap"><table><thead><tr><th>MEMBER</th><th>MEMBER ID</th><th>REGION</th><th>AGE</th><th>STATUS</th><th>PAYMENT</th><th>EXPIRY</th><th></th></tr></thead><tbody>
     {list.map(m=>{const p=latestPaymentByMember.get(m.id);return <tr key={m.id}><td><b>{m.firstName} {m.lastName}</b><br/><small>{m.email||m.phone||"—"}</small></td><td className="mono">{m.id}</td><td>{m.membershipLocation||"—"}</td><td>{ageFromDob(m.dateOfBirth)??"—"}</td><td><Badge status={effectiveStatus(m)}/></td><td>{p?<Badge status={p.status}/>:<span>—</span>}</td><td>{m.expiresAt||"—"}</td><td><button className="text-button" onClick={()=>setSelected(m)}>View →</button></td></tr>})}
    </tbody></table>{!list.length&&<div className="empty">{loading?"Loading members…":"No matching members."}</div>}</div></section>
   </>}
